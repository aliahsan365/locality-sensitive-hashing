...
const int debug_time = 0; // only times and similarty values
...
int main(void) {
	...
	//Reading Documents
     if (generation) {generating_docs();}
		 ...
		 cout  << "Se ejecutara con los documentos que se tengan generados de antemano" << endl;
		...
		for (float t : t_values) {
			for (size_t r = 1; r < 7; ++r) {
				...
				for (auto& kp : pairconcidence){ 
					if(debug) cout << " documentos " << kp.first+1 << " " << kp.second+1; 
					time_start = clock();
